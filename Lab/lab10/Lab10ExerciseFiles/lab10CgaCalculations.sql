create or replace procedure lab10CgaCalculations authid current_user as 
	/* DECLARATION SECTION */
    ...
    honours_student_exception   exception;
	...
	for studentRecord in studentCursor loop
		...
		begin
			...
			if studentCga >= honoursCga then
                raise honours_student_exception;
            end if;
			...
			if studentCga <= studentLowCga then
				...
				dbms_output.put_line('*** Low CGA alert for ' || studentRecord.firstName || ' ' ||
                    studentRecord.lastName || ' (' || currentStudentId || ') with cga ' ||
                    trim(to_char(studentCga, '9.99')) || '.');
			end if;	
...
exception 
			...
			when ZERO_DIVIDE then
                dbms_output.put_line('### ' || studentRecord.firstName || ' ' ||
                    studentRecord.lastName || ' (' || currentStudentId ||
                    ') is not enrolled in any course.');
            
			/* TODO: Handle honours student exception */
			when honours_student_exception then
                dbms_output.put_line('>>> ' || studentRecord.firstName || ' ' ||
                    studentRecord.lastName || ' (' || currentStudentId || ') with CGA=' ||
                    trim(to_char(studentCga, '9.99')) || ' is an honours student.');
            
		end;
	end loop; -- For processing each Student record
end lab10CgaCalculations;